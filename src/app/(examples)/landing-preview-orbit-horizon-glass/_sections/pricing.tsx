			...
			<div className='mx-auto max-w-5xl'>
				...
				<div className='grid grid-cols-1 gap-4 lg:grid-cols-3'>
					{tiers.map(t => (
						<article
							key={t.name}
							className={`ohg-reveal flex h-full flex-col ohg-glass rounded-2xl p-6 sm:p-7${t.featured ? ' ohg-glass-strong' : ''}`}
						>
							<div className='flex items-center justify-between'>
								...
								{t.featured ? (
									<span className='border border-[var(--line)] px-2.5 py-0.5 font-[family-name:var(--font-mono)] text-[10px] uppercase tracking-widest [color:var(--ink)]'>
										Popular
									</span>
								) : null}
							</div>
							...
							<a
								href='#trial'
								className={`group mt-7 inline-flex min-h-12 cursor-pointer items-center justify-center gap-2 ohg-glass rounded-2xl px-5 font-[family-name:var(--font-body)] font-medium text-sm ${
									t.featured
