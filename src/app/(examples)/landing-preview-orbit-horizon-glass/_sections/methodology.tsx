	...
	return (
		<section id='methodology' className='px-5 py-20 sm:px-8 sm:py-28'>
			<div className='mx-auto max-w-5xl'>
				...
				<h2 className='ohg-reveal font-[family-name:var(--font-display)] font-semibold text-[clamp(1.9rem,4.5vw,3rem)] leading-[1.06] tracking-[-0.02em] [color:var(--ink)]'>
					Numbers you can{' '}
					<span className='italic [color:var(--olive)]'>
						defend in a boardroom
					</span>
				</h2>
				<p className='ohg-reveal mt-4 max-w-2xl text-sm leading-relaxed [color:var(--mute)]'>
				...
				<div className='ohg-reveal mt-12 grid gap-0 ohg-glass rounded-2xl sm:grid-cols-3'>
					{pillars.map((p, i) => (
						<article
							key={p.title}
							className='relative p-5 sm:p-6 [&:not(:last-child)]:border-[var(--line)] [&:not(:last-child)]:border-b sm:[&:not(:last-child)]:border-r sm:[&:not(:last-child)]:border-b-0'
						>
				...
				<div className='ohg-reveal mt-10 ohg-glass rounded-2xl border-l-2 border-l-[color-mix(in_oklch,var(--olive)_45%,var(--line))] p-4 font-[family-name:var(--font-mono)] text-xs leading-relaxed [color:var(--mute)] sm:p-5'>
