...
export function SiteSwitcher() {
	...
	return (
		<section className='rounded-xl border border-[var(--line)] p-4 shadow-[0_1px_0_0_var(--line)] [background:var(--card)]'>
					...
					<div className='absolute top-full right-0 left-0 z-10 mt-1 overflow-hidden rounded-lg border border-[var(--line)] shadow-lg [background:var(--surface)]'>
						{SITES.map(site => (
							<li key={site.domain}>
								<button
									type='button'
									className='flex w-full items-center justify-between gap-2 px-3 py-2.5 text-left text-xs hover:[background:var(--card)]'
									role='option'
								...
								>
									...
									<span className='flex items-center gap-2 [color:var(--mute)]'>
										{site.co2}g
										{site.active ? (
											<Check className='h-3.5 w-3.5 [color:var(--olive)]' />
										) : null}
									</span>
								</button>
							</li>
