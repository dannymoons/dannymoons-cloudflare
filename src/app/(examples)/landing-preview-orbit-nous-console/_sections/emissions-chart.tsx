...
export function EmissionsChart() {
	...
	return (
		<section className='overflow-hidden rounded-xl border border-[var(--line)] shadow-[0_1px_0_0_var(--line)] [background:var(--card)]'>
			...
			<div className='flex flex-wrap items-baseline gap-3 px-4 pt-4 sm:px-5'>
				...
				<span className='rounded-full border border-[color-mix(in_oklch,var(--sage)_35%,transparent)] px-2 py-0.5 text-[10px] [color:var(--sage)]'>
					−48% YoY
				</span>
			</div>
						...
						return (
							<g key={months[i]}>
								...
								<circle
									...
									cy={y}
									r={active ? 4 : 2.5}
									fill={active ? 'var(--sage)' : 'var(--olive)'}
								/>
								{active ? (
									<text
										x={x}
