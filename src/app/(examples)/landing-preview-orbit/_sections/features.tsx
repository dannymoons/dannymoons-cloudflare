...
export function Features() {
	return (
		<section
			id='features'
			className='border-[var(--line)] border-y px-5 py-20 [background:color-mix(in_oklch,var(--surface)_25%,transparent)] sm:px-8 sm:py-28'
		>
			<div className='mx-auto max-w-5xl'>
				...
				<div className='grid gap-4 sm:grid-cols-2 lg:grid-cols-3'>
					{features.map(f => (
						<article
							key={f.title}
							className='ob-reveal group rounded-2xl border border-[color-mix(in_oklch,var(--orbit)_10%,var(--line))] p-5 transition-all duration-300 [background:color-mix(in_oklch,var(--panel)_55%,transparent)] hover:border-[color-mix(in_oklch,var(--orbit)_30%,var(--line))] hover:shadow-[0_0_32px_-14px_var(--orbit)] sm:p-6'
						>
							<span className='grid h-10 w-10 place-items-center rounded-lg border border-[color-mix(in_oklch,var(--orbit)_20%,var(--line))] transition-colors group-hover:[background:color-mix(in_oklch,var(--orbit)_12%,transparent)]'>
								<f.icon className='h-5 w-5 [color:var(--orbit)]' />
							</span>
