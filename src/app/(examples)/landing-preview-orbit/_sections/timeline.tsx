...
export function Timeline() {
	return (
		<section className='border-[var(--line)] border-y px-5 py-20 [background:color-mix(in_oklch,var(--surface)_30%,transparent)] sm:px-8 sm:py-28'>
			<div className='mx-auto max-w-5xl'>
				...
				<p className='ob-reveal mt-3 max-w-xl text-sm [color:var(--mute)]'>
				...
				<div className='ob-reveal mt-12 grid gap-4 sm:grid-cols-3'>
					{views.map(v => (
						<article
							key={v.id}
							className={`flex flex-col rounded-2xl border p-5 sm:p-6 ${
								v.labelAccent
									? 'border-[color-mix(in_oklch,var(--orbit)_35%,var(--line))] shadow-[0_0_40px_-16px_var(--orbit)] [background:color-mix(in_oklch,var(--panel)_85%,transparent)]'
						...
						>
							...
							<p className='mt-4 rounded-lg border border-[var(--line)] px-3 py-2 font-[family-name:var(--font-body)] text-[11px] [background:var(--void)] [color:var(--mint)]'>
								{v.sample}
							</p>
						</article>
					))}
				...
				<div className='ob-reveal mt-10 flex items-center gap-3 overflow-x-auto pb-2'>
					{(
						[
							'Jan',
							'Feb',
						...
						>
							<div
								className='w-1 rounded-full [background:var(--orbit)]'
								style={{
									height: `${24 + ((i * 7) % 40)}px`,
									opacity: 0.3 + i * 0.05
								}}
							/>
							<span className='text-[9px] [color:var(--mute)]'>{month}</span>
						</div>
					))}
				</div>
			</div>
