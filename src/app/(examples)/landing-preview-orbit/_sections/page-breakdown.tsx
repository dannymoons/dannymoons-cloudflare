...
function TrendIcon({ trend }: { trend: Trend }) {
	...
	if (trend === 'down')
		return <ArrowDown className='h-3.5 w-3.5 [color:var(--mint)]' />
	...
	return (
		<section className='px-5 py-20 sm:px-8 sm:py-28'>
			<div className='mx-auto max-w-5xl'>
				...
				<div className='ob-reveal mt-10 overflow-x-auto rounded-2xl border border-[color-mix(in_oklch,var(--orbit)_12%,var(--line))] [background:color-mix(in_oklch,var(--panel)_60%,transparent)]'>
					<table className='w-full min-w-[640px] text-left text-sm'>
						<thead>
							<tr className='border-[var(--line)] border-b text-[10px] uppercase tracking-[0.18em] [color:var(--mute)]'>
						...
						<tbody>
							{pages.map(p => (
								<tr
									key={p.path}
									className='border-[var(--line)] border-b transition-colors last:border-0 hover:[background:color-mix(in_oklch,var(--surface)_50%,transparent)]'
								>
									...
									<td className='px-4 py-4 sm:px-6'>
										<div className='flex h-8 w-24 items-end gap-0.5'>
											{p.spark.map(h => (
												<div
													key={`${p.path}-spark-${h}`}
													className='flex-1 rounded-sm [background:var(--orbit)]'
													style={{
														height: `${(h / 55) * 100}%`,
