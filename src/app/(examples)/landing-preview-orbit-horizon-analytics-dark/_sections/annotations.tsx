		...
		<section className='border-2 border-[var(--stroke)] shadow-[4px_4px_0_var(--shadow)] p-4 [background:var(--panel)]'>
			...
			>
				<textarea
					...
					rows={2}
					className='mb-2 w-full resize-none rounded-lg border border-[var(--line)] bg-transparent p-2 text-[11px] outline-none focus:border-[var(--stroke-strong)] focus:shadow-[3px_3px_0_var(--shadow)]'
				/>
				<button
					type='submit'
					className='w-full rounded-lg py-1.5 font-medium text-[11px] [background:var(--surface)] hover:[background:color-mix(in_oklch,var(--olive)_15%,var(--surface))]'
				>
					Add annotation
				</button>
			</form>
