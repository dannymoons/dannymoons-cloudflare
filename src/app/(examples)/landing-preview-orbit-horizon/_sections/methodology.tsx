	...
	return (
		<section id='methodology' className='px-5 py-20 sm:px-8 sm:py-28'>
			<div className='mx-auto max-w-5xl'>
				...
				<h2 className='oh-reveal font-[family-name:var(--font-display)] font-semibold text-[clamp(1.9rem,4.5vw,3rem)] leading-[1.06] tracking-[-0.02em] [color:var(--ink)]'>
					Numbers you can{' '}
					<span className='italic [color:var(--olive)]'>
						defend in a boardroom
					</span>
				</h2>
				<p className='oh-reveal mt-4 max-w-2xl text-sm leading-relaxed [color:var(--mute)]'>
				...
				<div className='oh-reveal mt-12 grid gap-0 border-2 border-[var(--ink)] sm:grid-cols-3'>
					{pillars.map((p, i) => (
						<article
							key={p.title}
							className='relative border-[var(--ink)] p-5 sm:p-6 [&:not(:last-child)]:border-b-2 sm:[&:not(:last-child)]:border-r-2 sm:[&:not(:last-child)]:border-b-0'
						>
				...
				<div className='oh-reveal mt-10 border-[var(--ink)] border-l-4 p-4 font-[family-name:var(--font-mono)] text-xs leading-relaxed [background:color-mix(in_oklch,var(--sage)_10%,var(--parchment))] [color:var(--mute)] sm:p-5'>
