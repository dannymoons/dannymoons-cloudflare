		...
		>
			<div className='mx-auto max-w-5xl'>
				<div className='mb-12 text-center'>
					...
					<p className='oh-reveal mx-auto mt-4 max-w-md text-sm [color:var(--mute)]'>
				...
				<div className='grid grid-cols-1 gap-4 lg:grid-cols-3'>
					{tiers.map(t => (
						<article
							key={t.name}
							className={`oh-reveal flex h-full flex-col border-2 p-6 sm:p-7 ${
								t.featured
									? 'border-[var(--ink)] [background:var(--panel)]'
									: 'border-[var(--ink)] [background:var(--panel)]'
							}`}
						>
							<div className='flex items-center justify-between'>
								...
								{t.featured ? (
									<span className='border border-[var(--ink)] px-2.5 py-0.5 font-[family-name:var(--font-mono)] text-[10px] uppercase tracking-widest [color:var(--ink)]'>
										Popular
									</span>
								) : null}
							</div>
							...
							<a
								href='#trial'
								className={`group mt-7 inline-flex min-h-12 cursor-pointer items-center justify-center gap-2 border-2 border-[var(--ink)] px-5 font-[family-name:var(--font-body)] font-medium text-sm ${
									t.featured
										? '[background:var(--ink)] [color:var(--parchment)]'
										: '[color:var(--ink)] hover:[background:var(--ink)] hover:[color:var(--parchment)]'
								}`}
							>
								Get started
