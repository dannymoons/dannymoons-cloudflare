	...
	return (
		<section className='rounded-xl border border-[var(--line)] p-4 [background:var(--panel)] sm:p-5'>
			<div className='mb-4 flex items-center justify-between'>
				...
				<span className='text-[10px] [color:var(--orbit)]'>
					3 recommendations
				</span>
			</div>
			<div className='grid gap-3 md:grid-cols-3'>
				{INSIGHTS.map(({ icon: Icon, title, impact, priority }) => (
					...
					>
						<div className='mb-2 flex items-center justify-between'>
							...
							<span
								className={`rounded px-1.5 py-0.5 text-[9px] uppercase ${
									priority === 'high'
										? '[background:color-mix(in_oklch,var(--orbit)_20%,transparent)] [color:var(--orbit)]'
										: '[color:var(--mute)]'
								}`}
							>
								{priority}
						...
						<p className='text-[11px] [color:var(--mint)]'>{impact}</p>
					</article>
				))}
			</div>
		</section>
