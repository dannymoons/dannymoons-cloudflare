...
export function DateRange() {
	...
	return (
		<section className='flex flex-col gap-3 rounded-xl border border-[var(--line)] p-4 [background:var(--panel)] sm:flex-row sm:items-center sm:justify-between'>
			<div className='flex items-center gap-2'>
				<Calendar className='h-4 w-4 [color:var(--orbit)]' aria-hidden />
					...
					<button
						...
						className={`rounded-lg px-3 py-1.5 font-medium text-[11px] transition-colors ${
							active === p.id
								...
								: 'border border-[var(--line)] [color:var(--mute)] hover:[color:var(--text)]'
						}`}
					>
						{p.label}
					</button>
