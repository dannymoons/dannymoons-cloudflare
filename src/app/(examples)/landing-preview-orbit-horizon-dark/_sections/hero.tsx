...
export function Hero() {
	return (
		<section className='relative px-5 pt-16 pb-20 sm:px-8 sm:pt-24 sm:pb-28'>
			<div
				aria-hidden
				className='pointer-events-none absolute inset-x-0 top-0 mx-auto h-72 opacity-60'
				style={{
					background:
			...
			<span className='ohd-reveal mx-auto flex max-w-fit items-center gap-2 rounded-full border border-[color-mix(in_oklch,var(--olive)_25%,var(--line))] px-3 py-1 font-[family-name:var(--font-mono)] text-[11px] uppercase tracking-[0.22em] [color:var(--mute)]'>
				<span className='h-1.5 w-1.5 animate-pulse rounded-full [background:var(--olive)]' />
				Page-level carbon tracking
			</span>

			<h1 className='ohd-reveal mx-auto mt-8 max-w-4xl text-balance text-center font-[family-name:var(--font-display)] font-semibold text-[clamp(2.5rem,7vw,5rem)] leading-[1.03] tracking-[-0.02em]'>
				See every page&rsquo;s{' '}
				<span className='italic [color:var(--olive)]'>carbon footprint</span>
			</h1>

			<p className='ohd-reveal mx-auto mt-6 max-w-xl text-balance text-center text-sm leading-relaxed [color:var(--mute)] sm:text-base'>
			...
			<div className='ohd-reveal mx-auto mt-10 max-w-3xl overflow-hidden border-2 border-[var(--stroke)] shadow-[6px_6px_0_var(--shadow)] [background:var(--panel)]'>
				...
				<div className='grid gap-px [background:var(--line)] sm:grid-cols-3'>
					{miniStats.map(s => (
						<div
							key={s.label}
							className='px-4 py-4 [background:var(--panel)] sm:px-5 sm:py-5'
						>
			...
			<div className='ohd-reveal mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row'>
				<a
					href='#trial'
					className='group inline-flex min-h-12 cursor-pointer items-center gap-2 border-2 border-[var(--stroke-strong)] px-6 font-[family-name:var(--font-body)] font-semibold text-sm [background:color-mix(in_oklch,var(--olive)_24%,var(--surface))] [color:var(--ink)]'
				>
					Start free trial
				...
				<a
					href='#dashboard'
					className='inline-flex min-h-12 cursor-pointer items-center gap-2 border-2 border-[var(--stroke)] px-6 font-[family-name:var(--font-body)] font-medium text-sm [color:var(--ink)] hover:border-[var(--stroke-strong)] hover:[background:color-mix(in_oklch,var(--olive)_14%,var(--surface))] hover:[color:var(--ink)]'
				>
					View live demo
				</a>
			</div>
