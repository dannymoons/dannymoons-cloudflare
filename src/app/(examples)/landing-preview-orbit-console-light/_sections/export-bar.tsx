	...
	return (
		<section className='rounded-xl border border-[var(--line)] p-4 [background:var(--panel)] sm:p-5'>
			...
			<div className='flex flex-col gap-3 sm:flex-row sm:items-end'>
						...
						{FORMATS.map(f => (
							<button
								...
								className={`flex-1 rounded-md py-1.5 font-medium text-[11px] transition-colors ${
									format === f
										? 'text-[var(--on-accent)] [background:var(--orbit)]'
				...
				<button
					type='button'
					className='h-9 shrink-0 rounded-lg px-4 font-medium text-[var(--on-accent)] text-xs [background:var(--orbit)] hover:opacity-90'
				>
					Download {format}
				</button>
			</div>
