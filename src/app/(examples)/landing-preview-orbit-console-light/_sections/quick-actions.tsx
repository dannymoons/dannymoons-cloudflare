			...
			<div className='grid gap-2 sm:grid-cols-2'>
				{ACTIONS.map(({ label, desc, icon: Icon }) => (
					<button
						key={label}
						type='button'
						className='flex items-start gap-3 rounded-lg border border-[var(--line)] p-3 text-left transition-colors hover:border-[color-mix(in_oklch,var(--orbit)_30%,var(--line))] hover:[background:var(--surface)]'
					>
						<span className='flex h-8 w-8 shrink-0 items-center justify-center rounded-lg [background:color-mix(in_oklch,var(--orbit)_15%,transparent)]'>
							<Icon className='h-4 w-4 [color:var(--orbit)]' aria-hidden />
						</span>
						<span>
