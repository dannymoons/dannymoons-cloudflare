	...
	return (
		<section className='rounded-xl border border-[var(--line)] p-4 [background:var(--panel)] sm:p-5'>
				...
				{ACTIONS.map(a => (
					<button
						...
						className={`w-full rounded-lg border px-3 py-2.5 text-left text-xs transition-colors ${
							selected === a.id
								? 'border-[color-mix(in_oklch,var(--orbit)_40%,var(--line))] [background:color-mix(in_oklch,var(--orbit)_10%,var(--surface))]'
			...
			<button
				type='button'
				disabled={!selected}
				className='mt-4 w-full rounded-lg py-2 font-medium text-[oklch(0.12_0.02_265)] text-xs [background:var(--orbit)] disabled:opacity-40'
			>
				Apply to selection
			</button>
		</section>
