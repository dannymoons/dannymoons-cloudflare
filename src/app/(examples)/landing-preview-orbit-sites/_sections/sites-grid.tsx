...
const STATUS: Record<string, { label: string; color: string }> = {
	...
	warn: { label: '2 alerts', color: 'var(--orbit)' },
				...
				{SITES.map(site => (
					<article
						...
						className={`rounded-xl border p-4 transition-colors ${
							selected.includes(site.id)
								? 'border-[color-mix(in_oklch,var(--orbit)_40%,var(--line))] [background:var(--surface)]'
					...
					>
						...
						<div className='mb-3 flex items-baseline gap-2'>
							...
							<span className='ml-auto rounded-md px-1.5 py-0.5 font-bold text-[10px] [background:color-mix(in_oklch,var(--orbit)_15%,transparent)] [color:var(--orbit)]'>
								{site.grade}
							</span>
						</div>
